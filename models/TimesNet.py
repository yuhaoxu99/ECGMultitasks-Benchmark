...
class TimesBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.k):
            ...
            remainder = (self.seq_len + self.pred_len) % period
            if remainder != 0:
...
class Model(nn.Module):
    ...
    def regression(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        # Normalization from Non-stationary Transformer
        # means = x_enc.mean(1, keepdim=True).detach()
        # x_enc = x_enc - means
        ...
        # 将输出展平并投影到一个单值上，作为回归结果
        enc_out = enc_out.view(enc_out.shape[0], -1)  # Flatten to [B, T * C]
        dec_out = self.projection(enc_out)  # [B, 1]
